...
public class OptionsController : MenuBase
{
	...
	[SerializeField]
	private AudioClip bongoCatAudioClip;
	...
	public static void ShowOptions(int loadGroup = 0)
	{
		if (!IsActive)
		{
			...
			CMInputCallbackInstaller.DisableActionMaps(typeof(OptionsController), from x in typeof(CMInput).GetNestedTypes()
				where x.IsInterface
				select x);
	...
	private IEnumerator CloseOptions()
	{
		...
		try
		{
			yield return StartCoroutine(Close(2f, optionsCanvasGroup));
			CMInputCallbackInstaller.ClearDisabledActionMaps(typeof(OptionsController), from x in typeof(CMInput).GetNestedTypes()
				where x.IsInterface
				select x);
			IsActive = false;
	...
	public void ToggleBongo(int bongoId)
	{
		...
		Settings.ManuallyNotifySettingUpdatedEvent("BongoCat", Settings.Instance.BongoCat);
	}
