public static class MultiCustomizationLauncher
{
	...
	public static void OpenMultiCustomization(DialogBox parent)
	{
		if (dialogBox == null)
		{
			dialogBox = PersistentUI.Instance.CreateNewDialogBox().WithTitle("MultiMapping", "multi.customize").DontDestroyOnClose();
			nameTextBox = dialogBox.AddComponent<TextBoxComponent>().WithLabel("MultiMapping", "multi.customize.name").WithInitialValue(Settings.Instance.MultiSettings.DisplayName)
				.WithMaximumLength(64);
			color = dialogBox.AddComponent<NestedColorPickerComponent>().WithLabel("MultiMapping", "multi.customize.color").WithInitialValue(Settings.Instance.MultiSettings.GridColor)
				.WithConstantAlpha(1f);
			serverUrlTextBox = dialogBox.AddComponent<TextBoxComponent>().WithLabel("MultiMapping", "multi.customize.server").WithInitialValue(Settings.Instance.MultiSettings.ChroMapTogetherServerUrl);
