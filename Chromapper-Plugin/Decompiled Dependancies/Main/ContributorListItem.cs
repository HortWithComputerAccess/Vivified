...
public class ContributorListItem : MonoBehaviour
{
	...
	[SerializeField]
	private TMP_InputField roleText;
	...
	public bool Dirty
	{
		get
		{
			if (!dirty && !(nameText.text != Contributor.Name) && !(Contributor.Role != roleText.text))
			{
				return Contributor.LocalImageLocation != imagePath;
			}
			return true;
		}
		set
	...
	{
		controller.ImageBrowser.BrowseForImage(SetImageLocation);
	}
