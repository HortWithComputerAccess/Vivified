...
public class MultiTimelineController
{
	...
	{
		if (!activeContainers.TryGetValue(identity, out var value))
		{
			value = Object.Instantiate(prefab, bookmarkManager.transform);
			value.Init(this, identity);
			activeContainers.Add(identity, value);
		}
		value.RefreshPosition(pose, timelineCanvas.sizeDelta.x + -20f, songLength);
	}
