...
public abstract class JSONNode
{
	...
	public static JSONNode Parse(string aJSON)
	{
		...
		for (; i < aJSON.Length; i++)
		{
			...
			case '}':
				...
				if (stack.Count == 0)
				{
					throw new JSONParseException("Too many closing brackets", stack2, stringBuilder.ToString());
				}
				stack.Pop();
				if (stack2.Any())
				{
				...
				{
					jSONNode.Add(text, ParseElement(stringBuilder.ToString(), flag2));
				}
				flag2 = false;
				flag4 = false;
				text = "";
			...
			case '"':
				flag = !flag;
				if (flag4 || (flag3 && stringBuilder.Length > 0 && flag))
				{
					throw new JSONParseException("Node missing a required comma", stack2, stringBuilder.ToString());
				}
				flag2 = flag2 || flag;
				break;
			case ',':
				...
				break;
			case '\n':
				if (flag3 && (!string.IsNullOrEmpty(text) || stringBuilder.Length > 0))
				{
					flag4 = true;
				}
				break;
		...
		if (flag)
		{
			throw new JSONParseException("Quotation marks seems to be messed up", stack2, stringBuilder.ToString());
		}
		if (jSONNode == null)
		{
	...
	public static implicit operator JSONNode(Color aColor)
	{
		...
		container.WriteColor(aColor, ColorContainerType == JSONContainerType.Array);
		return container;
	}
	...
	public Color ReadColor(Color aDefault)
	{
		if (IsObject)
		{
			return new Color(this["r"].AsFloat, this["g"].AsFloat, this["b"].AsFloat, HasKey("a") ? this["a"].AsFloat : 1f);
		}
		if (IsArray)
		{
			return new Color(this[0].AsFloat, this[1].AsFloat, this[2].AsFloat, (Count > 3) ? this[3].AsFloat : 1f);
		}
		return aDefault;
	}
